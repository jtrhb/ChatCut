...
import { calculateTotalDuration } from "../utils/timeline-utils";
import {
	AddTrackCommand,
	RemoveTrackCommand,
...
import { BatchCommand, PreviewTracker } from "../commands";
...
export class TimelineManager {
	...
	upsertKeyframes({
		...
		const commands = keyframes.map(
			(kf) => new UpsertKeyframeCommand(kf),
		);
		const command =
			commands.length === 1 ? commands[0] : new BatchCommand(commands);
		this.editor.command.execute({ command });
	...
	removeKeyframes({
		...
		const commands = keyframes.map(
			(kf) => new RemoveKeyframeCommand(kf),
		);
		const command =
			commands.length === 1 ? commands[0] : new BatchCommand(commands);
		this.editor.command.execute({ command });
