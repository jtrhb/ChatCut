...
| 2.5 | Wire `commitMutation` at the route boundary (revised from §2.5 original) | `apps/agent/src/routes/commands.ts` | The original plan called for replacing `serverCore.executeHumanCommand` directly with `commitMutation`. Implementation kept ServerEditorCore as a pure in-memory primitive (no projectId / no DB injection) and routed `commitMutation` at the `/commands` HTTP boundary instead. The route now requires a `commandFactory` dep so JSON requests are turned into real Command instances before hitting commitMutation; without one (the current production state) the route returns 400. The agent-command path is implemented + unit-tested but not yet called from MasterAgent tool dispatch — that wiring is deferred. |
