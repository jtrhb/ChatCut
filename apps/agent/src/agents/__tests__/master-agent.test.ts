...
function makeMockRuntime() {
	const executorRef: {
		current: ((name: string, input: unknown) => Promise<unknown>) | null;
	} = {
		current: null,
	};
	...
	return {
		...
		setToolExecutor: vi.fn(
			(fn: (name: string, input: unknown) => Promise<unknown>) => {
...
describe("MasterAgent", () => {
	...
	it("handleToolCall routes dispatch_editor to the editor dispatcher", async () => {
		...
		expect(result).toEqual(
			expect.objectContaining({ result: "editor result" }),
		);
	});
	...
	it("handleToolCall routes dispatch_vision to the vision dispatcher", async () => {
		...
		expect(result).toEqual(
			expect.objectContaining({ result: "vision result" }),
		);
	});
	...
	it("propose_changes returns error when ChangesetManager not configured", async () => {
		...
		expect(result).toEqual(
			expect.objectContaining({
				error: expect.stringContaining("ChangesetManager not configured"),
			}),
		);
	});
	...
	it("explore_options returns queued status", async () => {
		...
		expect(result).toEqual(
			expect.objectContaining({
				error: expect.stringContaining("ExplorationEngine not configured"),
			}),
		);
	});
	...
	it("routes dispatch_verification through DISPATCH_ROUTES", async () => {
		...
		const result = await (agent as any).handleToolCall(
	...
	describe("B3: per-dispatch taskId + rollback on throw", () => {
		...
		it("mints a DIFFERENT taskId for each dispatch", async () => {
			...
			const id1 = (editorDispatcher.mock.calls[0][0] as DispatchInput).identity
				?.taskId;
			const id2 = (editorDispatcher.mock.calls[1][0] as DispatchInput).identity
				?.taskId;
		...
		it("B4: loadMemories is called with resolved TaskContext (brand, series, projectId, sessionId, agentType)", async () => {
			...
			contextManager.registerBrand("proj-1", {
				brand: "acme",
				series: "spring",
			});
		...
		it("B4: getCurrentInjectedMemoryIds is cleared in the finally block after a turn", async () => {
			...
			await master.handleUserMessage("x", undefined, {
				projectId: "proj-1",
				sessionId: "s",
			});
			...
			expect(master.getCurrentInjectedMemoryIds()).toEqual({
				memoryIds: [],
				skillIds: [],
			});
		});
		...
		it("B4 [C2 fix]: each dispatch calls loadMemories with its own agentType and appends IDs", async () => {
			...
			const agentTypesLoaded = loadMemoriesSpy.mock.calls.map(
				(c) => (c[0] as any).agentType,
			);
			...
			expect((dispatchArg.context as any)?.memoryPromptText).toBe(
				"memory for editor",
			);
		});
		...
		it("B4 [C2 fix]: propose_changes after a dispatch stamps BOTH master and sub-agent memory IDs", async () => {
			...
			const { ServerEditorCore } = await import(
				"../../services/server-editor-core.js"
			);
			const { ChangesetManager } = await import(
				"../../changeset/changeset-manager.js"
			);
			const emptyState = {
				project: null,
				scenes: [],
		...
		it("B4 [C1 fix]: propose_changes during the turn stamps loaded memory IDs onto the changeset", async () => {
			...
			runtime.run.mockImplementationOnce(async () => {
				...
				expect(stored.injectedMemoryIds).toEqual([
					"mem-loaded-A",
					"mem-loaded-B",
				]);
		...
		it("B4: injects memory promptText into the system prompt", async () => {
			...
			expect(runArgs.system).toContain(
				"User prefers jump cuts on sports edits",
			);
		});
	...
	// ── Phase 1A: ContextSynchronizer injection ──────────────────────────────
	// Spec §3.11 / §7.4: each turn, MasterAgent must check the Change Log for
	// committed entries since the previous turn (other agents, human edits via
	// SSE) and prepend a summary to the current user message so the model has
	// a chance to react to out-of-band state changes. Audit §A.8 flagged that
	// ContextSynchronizer existed but had zero callers.
	describe("Phase 1A: ContextSynchronizer injection", () => {
		it("prepends buildContextUpdate output to the user message when wired", async () => {
			const buildContextUpdate = vi.fn(
				() => "## Context update (1 change)\n- [Human] deleted clip-3",
			);
			const contextSynchronizer = { buildContextUpdate } as any;

			const a = new MasterAgent({
				runtime: runtime as any,
				contextManager,
				writeLock,
				subAgentDispatchers: dispatchers,
			...
			await a.handleUserMessage("trim 5 seconds off the start");

			expect(buildContextUpdate).toHaveBeenCalledWith("master");
			expect(runtime.run).toHaveBeenCalled();
			const callArgs = (runtime.run as any).mock.calls[0];
			// run(config, input, history?) — input is the second arg
			const message = callArgs[1] as string;
			expect(message).toContain("## Context update");
			expect(message).toContain("deleted clip-3");
			expect(message).toContain("trim 5 seconds off the start");
			// the context update must appear before the actual request so the
			// model reads the state delta before deciding what to do
			expect(message.indexOf("## Context update")).toBeLessThan(
				message.indexOf("trim 5 seconds off the start"),
			);
		});

		it("passes the original message unchanged when buildContextUpdate returns null", async () => {
			const buildContextUpdate = vi.fn(() => null);
			const contextSynchronizer = { buildContextUpdate } as any;
			...
			await a.handleUserMessage("trim 5 sec");

			const message = (runtime.run as any).mock.calls[0][1] as string;
			expect(message).toBe("trim 5 sec");
		});

		it("works without a contextSynchronizer (backward compatible)", async () => {
			// Existing call sites (and many tests in this file) construct
			// MasterAgent without contextSynchronizer. The wiring must stay
			// optional so we don't break older deployments.
			await agent.handleUserMessage("trim 5 sec");
