...
describe("serializeEvent", () => {
	...
	it("produces correct { event, data } pair", () => {
		...
		// Phase 5b: `type` IS now duplicated into the JSON data so consumers
		// reading via `EventSource.onmessage` (existing apps/web/src/hooks/use-chat.ts
		// pattern that discriminates on `data.type`) can still see the event
		// type. The SSE `event:` line is still authoritative for
		// `addEventListener` consumers; this just makes both delivery
		// idioms work off one serializer.
